...
inflation <- read.csv(path_inflation,
                      skip = 4,  # Ajusta este número: omite las primeras 2 filas
                      header = TRUE)
...
inflation_largos <- inflation_largos %>% select('Country.Name','anio','inflacion')
...
gdp_largos  <- gdp %>% pivot_longer(cols= 5:69 ,
                                    names_to = "anio",               # Nombre de la nueva columna
                                    values_to = "gdp",        # Nombre de la columna de valores
                                    names_transform = list(anio = as.numeric)  # Convertir años a numérico
)
#seleccionar variables 
gdp_largos <- gdp_largos %>% select('Country.Name','anio','gdp')
...
unemp_largos  <- unemp %>% pivot_longer(cols= 5:69 ,
                                        names_to = "anio",               # Nombre de la nueva columna
                                        values_to = "unemp",        # Nombre de la columna de valores
                                        names_transform = list(anio = as.numeric)  # Convertir años a numérico
)
#seleccionar variables 
unemp_largos <- unemp_largos %>% select('Country.Name','anio','unemp')
